...
`<head>` ในตัวอย่างขั้นต้นมีแค่ `charset`, `viewport` และ `title` ซึ่งพอต่อการแสดงผลและผ่าน checkpoint ของบทนี้แล้ว

ถ้าเหลือเวลา อาจเพิ่ม `description` เพื่อให้เครื่องมือค้นหาใช้ข้อความสั้น ๆ ใต้ชื่อหน้า:

```html
<meta name="description" content="กาแฟบ้านดอย — กาแฟจากเกษตรกรในชุมชน คั่วสดทุกสัปดาห์">
```

::: tip ไอคอนแท็บ (favicon) เป็นทางเลือก
เว็บจริงมักมี `<link rel="icon" href="images/favicon.svg">` แต่ในเวิร์กชอปนี้ยังไม่มีไฟล์ favicon ในโฟลเดอร์งาน — **ไม่ต้องเพิ่ม** จนกว่าจะเตรียมไฟล์ไอคอนจริงไว้ใน `images/`
:::

::: tip ลำดับใน `<head>` มีผลอยู่บ้าง
`<meta charset>` ควรอยู่บนสุด เพราะ Browser ต้องรู้การเข้ารหัสก่อนอ่านส่วนที่เหลือ ส่วน `<title>` วางตามหลังได้ตามสะดวก
:::
...
ไปที่ [Tag พื้นฐาน](/html/tags) เพื่อเติมเมนูและหัวข้อย่อย
